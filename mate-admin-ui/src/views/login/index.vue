	...
	<div class="login_bg">
		<div class="login_adv" style="background-image: url(img/auth_banner.jpg);">
			...
			<div class="login_adv__bottom">
				© {{ mateConfig.APP_NAME }} {{ mateConfig.APP_VER }}
			</div>
		</div>
		<div class="login_main">
			...
			<div class="login-form">
				<div class="login-header">
					<div class="logo">
						<img :alt="mateConfig.APP_NAME" src="/img/logo.png">
						<label>{{ mateConfig.APP_NAME }}</label>
					</div>
				</div>
				<el-tabs>
				...
				<template v-if="mateConfig.MY_SHOW_LOGIN_OAUTH">
...
const config = reactive({
	lang: store.globalStore.language || mateConfig.LANG,
	dark: cache.getDark()
})
...
onMounted(() => {
	...
	store.viewTagsStore.clearViewTags()
	store.keepAliveStore.clearKeepLive()
	store.iframeStore.clearIframeList()
	console.log('%c MateAdmin %c MateBoot商业版: https://doc.mate.vip', 'background:#666;color:#fff;border-radius:3px;', '')
})


// https://cn.vuejs.org/guide/essentials/watchers.html#basic-example
// 此外lang变量为对象，只需要取value值即可，否则会报el错误
watch([() => config.dark, () => config.lang.value], ([darkVal, langVal]) => {
	if (darkVal) {
		document.documentElement.classList.add("dark")
		cache.setDark(darkVal)
	} else {
		document.documentElement.classList.remove("dark")
		cache.removeDark()
	}
	// 检查是否一致，不一致则更新
	let currentLangVal = i18n.global.locale.value;
	if (langVal && langVal !== currentLangVal) {
		i18n.global.locale.value = langVal
		cache.setLanguage(langVal)
	}
})
...
.login-form:deep(.login-msg-code) {
	display: flex;
	width: 100%;
}

.login-form:deep(.login-msg-code) .el-button {
	margin-left: 10px;
	--el-button-size: 42px;
}
