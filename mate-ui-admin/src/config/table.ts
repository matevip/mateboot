...
export default {
   ...
   * @column 用户配置好的列
   */
  columnSettingSave: function (tableName: any, column: any) {
    return new Promise((resolve) => {
      setTimeout(() => {
        //这里为了演示使用了session和setTimeout演示，开发时应用数据请求
        SessionStorage.setItem(tableName, column);
        resolve(true);
      }, 1000);
    });
  },
   ...
   */
  columnSettingGet: function (tableName: any, column: any) {
    return new Promise((resolve) => {
      //这里为了演示使用了session和setTimeout演示，开发时应用数据请求
      const userColumn = SessionStorage.getItem(tableName);
      if (userColumn) {
        resolve(userColumn);
      } else {
        resolve(column);
   ...
   * @column 组件接受到的props->column
   */
  columnSettingReset: function (tableName: any, column: any) {
    return new Promise((resolve) => {
      //这里为了演示使用了session和setTimeout演示，开发时应用数据请求
      setTimeout(() => {
